...
public class DBAccount {
    ...
    public Account getAccount(String userName, String password) {
        try {
            String sql = "Select * from AccountLogin where userName=? and password=?";
    ...
    public boolean checkExistWithUsername(String username){
        try {
            String sql = "Select * from AccountLogin where userName=?";
            PreparedStatement st = con.prepareStatement(sql);
            st.setString(1, username);
            ResultSet rs = st.executeQuery();
            while (rs.next()) {
    ...
    public void addAcount(String username, String password, String Fname, String Lname){
        try {
            String sql = "INSERT dbo.AccountLogin (userName, password, firstName, lastName) VALUES(?, ?, ?, ?)";
            PreparedStatement st = con.prepareStatement(sql);
            st.setString(1, username);
            st.setString(2, password);
